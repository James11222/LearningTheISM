...
parser = argparse.ArgumentParser(description='4PCF Computation Script')
...
def compute_NPCF_Coefficients(nbins, m_max):
    ...
    for i in tqdm(range(len(MHD_data))):
        for slice_ in range(0,256, 5):
            ...
            proj_4PCF = sarabande.measure(nPCF=4, projected=True, m_max=m_max, 
    ...
    proj_4PCF_coefficients_real, proj_4PCF_coefficients_imag = [], []
    for i in range(len(proj_4PCF_coefficients)):
        proj_4PCF_coefficients_real.append(proj_4PCF_coefficients[i].real)
        proj_4PCF_coefficients_imag.append(proj_4PCF_coefficients[i].imag)
        
    return proj_4PCF_coefficients_real, proj_4PCF_coefficients_imag

def save_coefficients(save_dir, proj_4PCF_coefficients_real, proj_4PCF_coefficients_imag):
    hf = h5py.File(f'{save_dir}/Coefficients_4PCF_real.h5', 'w')
    hf.create_dataset('data', data=proj_4PCF_coefficients_real)
    hf.close()
    
    hf = h5py.File(f'{save_dir}/Coefficients_4PCF_imag.h5', 'w')
    hf.create_dataset('data', data=proj_4PCF_coefficients_imag)
    hf.close()
...
if __name__ == "__main__":
    ...
    print("Measuring 4PCF Coefficients ...")
